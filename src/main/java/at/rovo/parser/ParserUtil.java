...
{
    /**
     * Splits a text into a sequence of tokens where <em>replace</em> is a set of characters that will split the
     * character sequence into tokens without including the splitting characters in the final token.
     * <p>
     * <em>nonReplace</em> defines characters that will split the text and include the token in the token before the
     * split, while <em>splitAndInclude</em> splits tokens and includes the splitting character with the following
     * token.
     *
     * @param text
    ...
    /**
     * Returns the node and all of its child nodes in a tree-like structure which can be used to inspect the correctness
     * of the parser.
     *
     * @param node
     *         The tag to start traversing through child nodes
     * @param endTagsIncluded
     *         Defines the list of parsed tokens contains end tags (true) or if they where omitted (false). In case they
     *         where omitted, this function does create closing tags automatically.
     *
     * @return A formatted representation of the HTML tree based on the selected root tag
     *
    ...
    /**
     * Returns the node and all of its child nodes in a TSReC structure which can be used to inspect the correctness of
     * the parser.
     *
     * @param nodes
    ...
    public static String niceTSReCFormat(List<Token> nodes)
    {
        ...
        for (Token node : nodes)
        {
            if (node instanceof Tag)
            {
                ...
                builder.append(" (");
                builder.append(tag.getNo());
                builder.append(", ");
                builder.append(tag.getEndNo());
                builder.append(", ");
                builder.append(tag.getParentNo());
                builder.append(", ");
                builder.append(tag.getLevel());
                builder.append(") : ");
     ...
     * <p>
     * Stemming is applied here: <ul> <li>Porter's stemming algorithm is applied to non-numeric words</li> <li>Numeric
     * words (numbers) are stemmed to 1</li> </ul>
     *
     * @param text
