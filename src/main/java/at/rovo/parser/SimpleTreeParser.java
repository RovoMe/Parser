 ...
 * closing tags as children to their corresponding parent node. Moreover each {@link Tag} node contains a unique ID as
 * well as a reference to the ID of its parent and its position in the siblings list of its parent.
 * <p>
 * Ancestor informations are collected through the utilization of a stack. A simple heuristic is used to probe if a
 * closing tag is suitable. The heuristic checks the stack for an opening tag that matches the closing tag. If there is
 * a matching tag on the stack the opening tag is used that was added the latest. This further implies that all tags
 * that are added after the opening tag are removed from the stack as well.
 * <p>
 * <em>SimpleTreeParser</em> allows adding tag names to the parser instance via {@link
 * #addTagToIgnoreIndentation(String)} which will ignore these tags from taking an effect on the indentation. The
 * subsequent tag will be on the same level as the declared tags that ignore indentation.
 *
...
public class SimpleTreeParser extends Parser
{
     ...
     * the same tree-level like the tag contained in this list
     **/
    protected List<String> ignoreIndentationTags = new ArrayList<>();
    ...
    /**
     * Add a tag which ignores indentation. This means the proximating tag will be on the same tree level like this
     * tag.
     *
     * @param tagName
    ...
    /**
     * Creates a new HTML tag and keeps track of ancestor information as well as the indentation level of the current
     * token.
     *
     * @param token
     ...
     *         If no matching opening tag can be found on the stack for a closing tag
     */
    @Override
            ...
            if (!this.needsRemoval(tag))
            {
                try
                {
                    ...
                    if (LOG.isDebugEnabled())
                    {
                        ...
                        LOG.debug("{} id: {} parent: {} html: \t\t{}", builder.toString() + newTag.getName(),
                                  newTag.getNo(), newTag.getParentNo(), newTag.getHTML());
                    }
                    ...
                    if (!this.ignoreIndentationTags.contains(newTag.getShortTag().toLowerCase()) &&
                        (stack.isEmpty() || newTag.isOpeningTag()) && !newTag.isInlineClosingTag())
                    {
                        stack.push(newTag);
                    }
    ...
    /**
     * Checks if a HTML node has a corresponding parent on the stack. If so nodes are taken from the stack until the
     * parent is reached. The parent is now the last entry on the stack.
     * <p>
     * If no matching parent could be found, the algorithm assumes that the tag itself is a wild tag and should not be
     * included in the final output, therefore the tag is removed from the tokenList and the reference of the parent
     * pointing to this node is removed.
     *
     * @param node
     ...
     *
     * @return Returns true if the element is a wild node and has no ancestor on the stack, false otherwise
     */
    protected boolean checkElementsOnStack(Tag node, Stack<Tag> stack)
    {
        ...
        LOG.warn("Ignoring {} {}", node.getNo(), node.getName());
        return true;
    }
}
